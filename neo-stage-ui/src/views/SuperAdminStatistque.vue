      ...
      <main class="canvas-engine flex-grow-1 overflow-auto custom-scrollbar">
        ...
        <div class="dashboard-view animate__animated animate__fadeIn p-4 p-lg-5">
          ...
          <header class="d-flex justify-content-between align-items-end mb-5 flex-wrap gap-3">
            <div>
              <div class="breadcrumb-pro mb-2">
                <span class="root">Super Administration</span>
                <i class="fa-solid fa-chevron-right mx-2 separator"></i>
                <span class="current">Terminal Analytique</span>
              </div>
              <h2 class="premium-title">Cognition &amp; <span class="gradient-text">Statistiques</span></h2>
              <div class="d-flex gap-3 mt-2 flex-wrap">
                ...
                <div class="status-pill" :class="systemStatus === 'OPTIMAL' ? 'status-ok' : 'status-warn'">
            ...
            <div class="d-flex gap-3 align-items-center flex-wrap">
              ...
              <div class="ai-companion-box">
                ...
                <div>
                  <div class="ai-name">EVALUA_AI_CORE</div>
                  <div class="ai-mode">OPTIMISATION ACTIVE</div>
                </div>
              </div>

              <div class="view-toggle-cluster">
                <button :class="['btn-view-toggle', { active: viewMode === 'overview' }]"   @click="viewMode = 'overview'"   title="Vue globale"><i class="fa-solid fa-chart-pie"></i></button>
                ...
                <button :class="['btn-view-toggle', { active: viewMode === 'system' }]"     @click="viewMode = 'system'"     title="Système"><i class="fa-solid fa-server"></i></button>
              </div>

              <button class="btn-refresh-pro" @click="refreshData" :disabled="refreshing" title="Rafraîchir">
          ...
          <div class="row g-4 mb-5">
            <div class="col-xl-3 col-md-6" v-for="stat in masterKpis" :key="stat.label">
          ...
          <div v-if="viewMode === 'overview'" class="animate__animated animate__fadeIn">
            ...
            <div class="row g-4 mb-4">
              <div class="col-lg-8">
                <div class="enigma-card p-4 h-100">
                  <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
                    <div>
                      <h6 class="fw-800 m-0">Performance des Flux d'Évaluation</h6>
                      <p class="text-muted small m-0">Scores moyens par campagne</p>
                    </div>
                    <div class="time-range-picker">
                      <button :class="['trp-btn', { active: timeRange === '24h' }]" @click="timeRange = '24h'">24H</button>
                      <button :class="['trp-btn', { active: timeRange === '7j' }]"  @click="timeRange = '7j'">7J</button>
                      <button :class="['trp-btn', { active: timeRange === '30j' }]" @click="timeRange = '30j'">30J</button>
                    </div>
                  </div>
                  ...
                  <div class="chart-stage">
                    ...
                    <div v-else-if="apiChartData.length === 0" class="chart-empty text-center text-muted py-5">
                      ...
                      <p class="small fw-700">Aucune donnée disponible.</p>
                    </div>
                    <div v-else class="glow-pillars-container">
                      <div v-for="(item, i) in apiChartData" :key="i" class="pillar-group">
                        ...
                        <div class="pillar-vessel">
                          <div class="pillar-fill" :style="{ height: item.score + '%', background: getPillarColor(item.score) }">
              ...
              <div class="col-lg-4">
                <div class="enigma-card p-4 h-100">
                  <h6 class="fw-800 mb-4">Infrastructure &amp; Ressources</h6>
                  <div class="resource-gauges">
                    <div class="gauge-item-pro mb-4" v-for="res in resources" :key="res.name">
                      <div class="d-flex justify-content-between align-items-center mb-2">
                        <span class="gauge-name">{{ res.name }}</span>
                        <span class="gauge-val" :style="{ color: getGaugeColor(res.usage) }">{{ res.usage }}%</span>
                      </div>
                      <div class="g-track">
                        <div class="g-fill-pro" :style="{ width: res.usage + '%', background: getGaugeColor(res.usage) }"></div>
                      </div>
                    </div>
                  </div>

                  <div class="talent-stream mt-4">
                    <h6 class="fw-800 mb-3" style="font-size:0.7rem;letter-spacing:1px;color:#94a3b8;">TOP TALENTS DÉTECTÉS</h6>
                    <div v-if="loading">
                      <div v-for="i in 3" :key="i" class="skeleton-talent mb-2"></div>
                    </div>
                    <div v-else-if="leaders.length === 0" class="text-center text-muted py-3 small">Aucun talent détecté.</div>
                    <div v-else class="talent-row-pro" v-for="(leader, idx) in leaders.slice(0, 5)" :key="idx">
                      <div class="talent-rank" :class="idx === 0 ? 'rank-gold' : idx === 1 ? 'rank-silver' : 'rank-bronze'">{{ idx + 1 }}</div>
            ...
            <div class="row g-4 mb-4">
              <div class="col-lg-4">
                <div class="enigma-card p-4">
                  <h6 class="fw-800 mb-4">Répartition des Organisations</h6>
                  <div class="donut-chart-container">
                    <svg viewBox="0 0 120 120" width="130">
                      ...
                      <text x="60" y="58" text-anchor="middle" class="donut-center-text">{{ statsData.totalEntreprises || 0 }}</text>
                      <text x="60" y="72" text-anchor="middle" class="donut-sub-text">ORGS</text>
                    </svg>
                    <div class="donut-legend">
                      <div v-for="seg in orgDonutSegments" :key="seg.label" class="donut-legend-item">
              ...
              <div class="col-lg-8">
                <div class="enigma-card p-4">
                  <h6 class="fw-800 mb-4">Activité Récente (7 jours)</h6>
                  <div class="bar-chart-v2">
                    <div v-for="(bar, i) in weekActivityData" :key="i" class="bar-col">
                      <div class="bar-wrap">
                        <div class="bar-fill bar-amber" :style="{ height: bar.sessions + '%' }" :title="`Sessions: ${bar.sessions}%`"></div>
                        <div class="bar-fill bar-indigo" :style="{ height: bar.users + '%' }" :title="`Utilisateurs: ${bar.users}%`"></div>
                      </div>
                  ...
                  <div class="d-flex gap-3 mt-3 justify-content-center">
                    <div class="d-flex align-items-center gap-2">
                      ...
                      <span class="small text-muted fw-700">Sessions</span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                      <span class="legend-dot dot-indigo"></span>
                      <span class="small text-muted fw-700">Utilisateurs</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div v-if="viewMode === 'analytics'" class="animate__animated animate__fadeIn">
              ...
              <div class="col-12">
                <div class="enigma-card p-4">
                  <div class="d-flex justify-content-between align-items-center mb-4">
                    <h6 class="fw-800 m-0">Analytique Profonde — Sessions & Scores</h6>
                    <div class="d-flex gap-2 align-items-center">
                      <span class="legend-dot dot-amber"></span><span class="small text-muted">Sessions</span>
                      <span class="legend-dot dot-indigo ms-3"></span><span class="small text-muted">Candidats</span>
                      <span class="legend-dot dot-green ms-3"></span><span class="small text-muted">Score Moyen</span>
                    </div>
                  </div>
            ...
            <div class="row g-4">
              <div class="col-lg-6">
                <div class="enigma-card p-4">
                  <h6 class="fw-800 mb-4">Métriques Détaillées</h6>
                  <div class="metrics-table">
                    <div class="metric-row" v-for="m in detailedMetrics" :key="m.label">
              ...
              <div class="col-lg-6">
                <div class="enigma-card p-4">
                  <h6 class="fw-800 mb-4">Journal d'Activité</h6>
                  <div class="activity-timeline" v-if="auditLogs.length > 0">
                    <div class="timeline-item" v-for="log in auditLogs.slice(0,5)" :key="log.id">
                      ...
                      <div class="tl-content">
                        ...
                        <p class="text-muted m-0" style="font-size:0.72rem">{{ log.action }} — {{ log.details }}</p>
                      </div>
                      <span class="tl-time">{{ formatTime(log.timestamp) }}</span>
                    </div>
                  </div>
                  <div v-else-if="loading" class="text-center py-4"><div class="spinner-pro-premium"></div></div>
                  <div v-else class="text-center text-muted py-4 small">Aucun log disponible.</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="viewMode === 'system'" class="animate__animated animate__fadeIn">
            <div class="row g-4 mb-4">
              <div class="col-12">
                <div class="system-health-banner p-4" :class="systemStatus === 'OPTIMAL' ? 'banner-ok' : 'banner-warn'">
                  <div class="d-flex align-items-center gap-4">
                    <div class="health-icon-ring">
                      <i class="fa-solid fa-shield-halved fa-2x" :class="systemStatus === 'OPTIMAL' ? 'text-success' : 'text-amber'"></i>
                    </div>
                    <div>
                      <h5 class="fw-900 m-0">Statut Système : {{ systemStatus }}</h5>
                      <p class="m-0 text-muted small">Dernière vérification : {{ lastSyncTime }}</p>
                    </div>
                    <div class="ms-auto d-flex gap-4">
                      <div class="sys-stat-box text-center">
                        <div class="sys-stat-val fw-900">{{ responseTime }}ms</div>
                        <div class="sys-stat-label">LATENCE</div>
                      </div>
                      <div class="sys-stat-box text-center">
                        <div class="sys-stat-val fw-900">99.8%</div>
                        <div class="sys-stat-label">UPTIME</div>
                      </div>
                      <div class="sys-stat-box text-center">
                        <div class="sys-stat-val fw-900">v6.5</div>
                        <div class="sys-stat-label">VERSION</div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="row g-4">
              <div class="col-lg-6">
                <div class="enigma-card p-4">
                  <h6 class="fw-800 mb-4"><i class="fa-solid fa-server me-2 text-amber"></i>Ressources Serveur</h6>
                  <div class="gauge-item-pro mb-4" v-for="res in extendedResources" :key="res.name">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                      <div class="d-flex align-items-center gap-2">
                        <i :class="res.icon + ' text-muted'" style="font-size:0.75rem"></i>
                        <span class="gauge-name">{{ res.name }}</span>
                      </div>
                      <span class="gauge-val fw-900" :style="{ color: getGaugeColor(res.usage) }">{{ res.usage }}%</span>
                    </div>
                    <div class="g-track">
                      <div class="g-fill-pro" :style="{ width: res.usage + '%', background: getGaugeColor(res.usage) }"></div>
                    </div>
              ...
              <div class="col-lg-6">
                <div class="enigma-card p-4">
                  <h6 class="fw-800 mb-4"><i class="fa-solid fa-plug me-2 text-indigo"></i>Services Actifs</h6>
                  <div class="services-list">
                    <div class="service-row" v-for="srv in activeServices" :key="srv.name">
                      <div class="service-status-dot" :class="srv.status === 'running' ? 'dot-ok' : 'dot-err'"></div>
                      <div class="service-info flex-grow-1">
                        <div class="fw-700 small">{{ srv.name }}</div>
                        <div class="text-muted" style="font-size:0.65rem">{{ srv.desc }}</div>
                      </div>
                      <div class="service-badge" :class="srv.status === 'running' ? 'badge-ok' : 'badge-err'">
                        {{ srv.status === 'running' ? 'ACTIF' : 'ARRÊTÉ' }}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="col-12">
                <div class="enigma-card p-4">
                  <div class="d-flex justify-content-between align-items-center mb-4">
                    <h6 class="fw-800 m-0"><i class="fa-solid fa-list-check me-2 text-amber"></i>Logs d'Audit Complets</h6>
                    <div class="search-inline-box" style="width:240px">
                      <i class="fa-solid fa-magnifying-glass"></i>
                      <input type="text" v-model="auditSearch" placeholder="Rechercher..." class="search-inline-input">
                    </div>
                  </div>
                  ...
                  <div v-else>
                    <div class="list-header-row d-flex align-items-center px-4 py-2 mb-2">
                      <span style="width:160px" class="list-col-label">UTILISATEUR</span>
                      <span class="flex-grow-1 list-col-label">ACTION</span>
                      <span style="width:200px" class="list-col-label">DÉTAILS</span>
                      <span style="width:120px" class="list-col-label text-center">HORODATAGE</span>
                    </div>
                    <div v-if="filteredAuditLogs.length === 0" class="text-center text-muted py-4 small">Aucun log trouvé.</div>
                    <div v-else v-for="log in filteredAuditLogs" :key="log.id" class="list-row-item d-flex align-items-center px-4 py-3 mb-2">
                      ...
                      <div style="width:120px" class="text-center small text-muted">{{ formatTime(log.timestamp) }}</div>
                    </div>
                  </div>
                </div>
              </div>
...
const masterKpis = ref([
  { label: 'ORGS ACTIVES',     value: '—', icon: 'fa-solid fa-building-shield', color: '#f59e0b', bg: '#fffbeb', trend: 12  },
  { label: 'TALENTS IA',       value: '—', icon: 'fa-solid fa-microchip',        color: '#6366f1', bg: '#eef2ff', trend: 8   },
  { label: 'SESSIONS FLOW',    value: '—', icon: 'fa-solid fa-wave-square',       color: '#10b981', bg: '#ecfdf5', trend: -2  },
  { label: 'SÉCURITÉ SYSTÈME', value: 'MAX', icon: 'fa-solid fa-shield-halved',   color: '#f43f5e', bg: '#fff1f2', trend: 5   },
]);
...
const extendedResources = ref([
  { name: 'CPU',              usage: 68,  icon: 'fa-solid fa-microchip' },
  { name: 'Mémoire RAM',      usage: 54,  icon: 'fa-solid fa-memory' },
  { name: 'Stockage Disque',  usage: 42,  icon: 'fa-solid fa-hard-drive' },
  { name: 'Bande Passante',   usage: 87,  icon: 'fa-solid fa-network-wired' },
  { name: 'Uptime Global',    usage: 99,  icon: 'fa-solid fa-server' },
]);

const activeServices = ref([
  { name: 'API Gateway',           desc: 'Routage & authentification',     status: 'running' },
  { name: 'EvaluaBot Engine',      desc: 'Moteur IA d\'évaluation',         status: 'running' },
  { name: 'Queue Processor',       desc: 'Traitement asynchrone des jobs',  status: 'running' },
  { name: 'Email Notification',    desc: 'Service d\'envoi automatique',    status: 'running' },
  { name: 'Analytics Aggregator',  desc: 'Collecte des métriques',          status: 'running' },
  { name: 'Backup Service',        desc: 'Sauvegarde planifiée toutes 6h',  status: 'stopped' },
]);
...
const detailedMetrics = computed(() => [
  { label: 'Taux de Réussite',   value: statsData.value.tauxReussite    || '—', pct: statsData.value.tauxReussite    || 74, icon: 'fa-solid fa-chart-line',       color: '#10b981', bg: '#ecfdf5' },
  { label: 'Candidats Actifs',   value: statsData.value.candidatsActifs || '—', pct: Math.min((statsData.value.candidatsActifs / 500) * 100, 100) || 60, icon: 'fa-solid fa-user-tie', color: '#6366f1', bg: '#eef2ff' },
  { label: 'Questions en Banque',value: statsData.value.totalQuestions  || '—', pct: Math.min((statsData.value.totalQuestions / 1000) * 100, 100) || 45, icon: 'fa-solid fa-database', color: '#f59e0b', bg: '#fffbeb' },
  { label: 'Sessions Terminées', value: statsData.value.sessionsTermin  || '—', pct: statsData.value.sessionsTermin  || 82, icon: 'fa-solid fa-flag-checkered',   color: '#f43f5e', bg: '#fff1f2' },
]);
...
const orgDonutSegments = computed(() => {
  ...
  const circ  = 283;
  const data  = [
    { label: 'Actives',    count: Math.ceil(total * 0.6),  color: '#f59e0b' },
    { label: 'Inactives',  count: Math.ceil(total * 0.25), color: '#6366f1' },
    { label: 'En attente', count: Math.floor(total * 0.15),color: '#10b981' },
  ];
...
const fetchData = async () => {
  ...
  try {
    ...
    if (auditRes.status === 'fulfilled') {
      ...
      if (auditLogs.value.length > 0) lastAudit.value = auditLogs.value[auditLogs.value.length - 1];
    }
    ...
    lastSyncTime.value = new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  }
};
...
const refreshData = async () => {
  ...
  await fetchData();
  showPulseToast('Données actualisées.', 'success', 'fa-solid fa-rotate');
};
...
const generateAllMocks = () => {
  ...
  statsData.value = { totalEntreprises: 12, totalUtilisateurs: 348, totalTests: 87, tauxReussite: 74, candidatsActifs: 210 };
  apiChartData.value = generateMockChart();
  leaders.value = [
...
/* ─── HELPERS ────────────────────────────────────────────────────── */
const getPillarColor   = (score) => score >= 80 ? '#10b981' : score >= 60 ? '#f59e0b' : '#f43f5e';
const getGaugeColor    = (pct)   => pct >= 90 ? '#f43f5e' : pct >= 70 ? '#f59e0b' : '#10b981';
const getAuditColor    = (action) => ({
  LOGIN: '#10b981', CREATE: '#6366f1', DEPLOY: '#f59e0b',
  DELETE: '#f43f5e', EXPORT: '#06b6d4'
}[action] || '#94a3b8');
const getAuditBadgeClass = (action) => ({
  LOGIN:  'audit-login',
  CREATE: 'audit-create',
  DEPLOY: 'audit-deploy',
  DELETE: 'audit-delete',
...
.bar-amber  { background: #f59e0b; } .bar-indigo { background: #6366f1; } .bar-green  { background: #10b981; }
.bar-label  { font-size: 0.6rem; font-weight: 800; color: #94a3b8; }
.legend-dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; }
.dot-amber  { background: #f59e0b; } .dot-indigo { background: #6366f1; } .dot-green  { background: #10b981; }

/* ── METRICS TABLE (analytics view) ── */
...
/* ════════════════════════════════════════════
   DARK MODE OVERRIDES — même pattern que Campagnes.vue
════════════════════════════════════════════ */
...
[data-theme="dark"] .metric-icon-box { opacity: 0.9; }
