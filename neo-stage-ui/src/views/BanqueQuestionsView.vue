  ...
  <div class="admin-layout d-flex min-vh-100">
    ...
    <div class="main-content flex-grow-1 p-4 p-lg-5 position-relative">
      ...
      <div class="row g-4 position-relative">
        ...
        <transition-group name="stagger" tag="div" class="row g-4 m-0 p-0">
            ...
            <div class="asset-card-cyber">
              <div class="card-header-actions">
                ...
                <div class="d-flex gap-2">
                  <button @click="openModal(q)" class="btn-card-action"><i class="fa-solid fa-pen"></i></button>
                  <button @click="handleDelete(q.id)" class="btn-card-action text-danger"><i class="fa-solid fa-trash"></i></button>
                </div>
              </div>
      ...
      <!-- MODALE : GESTIONNAIRE DE CATÉGORIES -->
      ...
      <!-- MODALE : CONFIGURATEUR DE QUESTION (MASTER) -->
      <transition name="modal-fade">
          ...
          <div class="modal-cyber-card master-config-modal animate__animated animate__zoomIn">
            ...
            <div class="modal-cyber-body custom-scrollbar">
              <div class="row g-4">
                ...
                <!-- ZONE RÉPONSES DYNAMIQUE (CHOIX UNIQUE / MULTIPLE / VRAI-FAUX) -->
                  ...
                  <div class="responses-list">
                    <div v-for="(rep, index) in form.reponses" :key="index" class="response-cyber-row animate__animated animate__fadeIn">
                      <div class="check-container" :title="rep.estCorrecte ? 'Bonne réponse' : 'Marquer comme correcte'">
                        <input :type="form.type === 1 ? 'checkbox' : 'radio'" 
                          name="master-correct" 
                          v-model="rep.estCorrecte"
                          :value="true"
                          @change="toggleCorrectness(index)"
                          class="cyber-check-input">
                      </div>
                      <input type="text" v-model="rep.texte" class="cyber-field-simple flex-grow-1" placeholder="Texte de la réponse...">
            ...
            <div class="modal-cyber-footer">
              ...
              <button @click="save" class="btn-primary-gradient px-5 shadow">
                  <i class="fa-solid fa-shield-halved me-2"></i> ENREGISTRER L'ACTIF
              </button>
            </div>
          </div>
        </div>
...
const form = reactive({ 
  id: '', texte: '', type: 0, poids: 1, categorie: '', reponses: [], bonneReponse: '' 
});
...
const toggleCorrectness = (index) => {
    if (form.type === 0 || form.type === 2) { // Unique choice
        form.reponses.forEach((r, i) => r.estCorrecte = i === index);
    }
};

const addResponse = () => form.reponses.push({ texte: '', estCorrecte: false });
const removeResponse = (index) => form.reponses.splice(index, 1);

const addCategory = () => {
    if (newCatName.value.trim() && !categoriesList.value.includes(newCatName.value)) {
        categoriesList.value.push(newCatName.value.trim());
        newCatName.value = '';
    }
};
...
const openModal = (q = null) => {
  ...
  if (q) {
      ...
      Object.assign(form, { 
          ...
          texte: '', type: 0, poids: 1, categorie: categoriesList.value[0], 
...
const save = async () => {
  try {
    ...
    fetchData();
  } catch (e) { alert("Erreur lors de la sauvegarde système."); }
};

const handleDelete = async (id) => {
    if (confirm("Supprimer définitivement cet actif de la banque ?")) {
...
.vault-text { font-weight: 700; color: #94a3b8; border-left: 2px solid #e2e8f0; padding-left: 10px; }
...
.btn-manage-cat { background: white; border: 1px solid #e2e8f0; padding: 12px 20px; border-radius: 16px; font-weight: 800; color: #64748b; font-size: 12px; transition: 0.3s; }
.btn-manage-cat:hover { border-color: #eab308; color: #eab308; transform: translateY(-2px); }

.btn-primary-gradient { 
    background: linear-gradient(135deg, #eab308 0%, #facc15 100%); color: white; border: none; 
    padding: 13px 25px; border-radius: 16px; font-weight: 800; font-size: 13px; transition: 0.3s;
}
...
.kpi-glass-card { 
    ...
    border: 1px solid white; border-radius: 26px; padding: 25px; 
    display: flex; align-items: center; gap: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.03);
}
...
/* FILTERS BAR */
.filter-cyber-bar { background: rgba(255, 255, 255, 0.7); backdrop-filter: blur(10px); border: 1px solid white; padding: 15px 25px; border-radius: 20px; display: flex; align-items: center; }
.filter-label { font-size: 11px; font-weight: 900; color: #94a3b8; margin-right: 15px; }
.chip { background: white; border: 1px solid #e2e8f0; padding: 8px 18px; border-radius: 14px; font-size: 12px; font-weight: 700; color: #64748b; transition: 0.3s; margin-right: 8px; }
...
/* ASSET CARDS */
.asset-card-cyber { 
    background: rgba(255, 255, 255, 0.9); border: 1px solid white; border-radius: 30px; 
    padding: 28px; height: 100%; transition: 0.4s; display: flex; flex-direction: column;
...
.btn-card-action { width: 34px; height: 34px; background: white; border: 1px solid #f1f5f9; border-radius: 10px; font-size: 12px; transition: 0.3s; color: #94a3b8; }
...
.btn-close-cyber { background: none; border: none; font-size: 32px; color: #cbd5e1; font-weight: 300; }
...
.btn-del-mini { background: none; border: none; color: #f87171; font-size: 18px; line-height: 1; }
...
.cyber-input-area, .cyber-field-simple, .cyber-select-field { width: 100%; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 14px 20px; font-weight: 600; font-size: 14px; outline: none; transition: 0.3s; }
.cyber-input-area:focus, .cyber-field-simple:focus { border-color: #eab308; background: white; }

.response-cyber-row { display: flex; align-items: center; gap: 15px; margin-bottom: 12px; }
.cyber-check-input { width: 22px; height: 22px; accent-color: #eab308; cursor: pointer; }
.btn-add-cyber { background: #0f172a; color: white; border: none; padding: 8px 18px; border-radius: 12px; font-weight: 800; font-size: 11px; transition: 0.3s; }
.btn-del-cyber { background: none; border: none; color: #f87171; font-size: 20px; }
...
.btn-glass-cancel { background: #f1f5f9; border: none; padding: 12px 25px; border-radius: 15px; font-weight: 800; color: #64748b; }

.cyber-range { accent-color: #eab308; width: 100%; height: 8px; border-radius: 10px; }
