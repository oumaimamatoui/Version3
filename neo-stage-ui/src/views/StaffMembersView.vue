      ...
      <main class="canvas-engine flex-grow-1 overflow-auto custom-scrollbar">
        <div class="p-4 p-lg-5">
          ...
          <header class="d-flex justify-content-between align-items-end mb-5 flex-wrap gap-3">
            <div>
              <div class="breadcrumb-pro mb-2">
                <span class="root">Ressources Humaines</span>
                <i class="fa-solid fa-chevron-right mx-2 separator"></i>
                <span class="current">Terminal du Personnel</span>
              </div>
              <h2 class="premium-title">
                Membres du <span class="gradient-text">Personnel</span>
              </h2>
              <p class="header-sub mt-1">
                Collaborateurs de :
                <span class="fw-900 text-dark">{{ authStore.user?.entrepriseNom || 'Ma Société' }}</span>
              </p>
            </div>
            <div class="d-flex gap-3 flex-wrap align-items-center">
              ...
              <div class="view-toggle-cluster">
                <button :class="['btn-view-toggle', { active: viewMode === 'table' }]" @click="viewMode = 'table'" title="Vue tableau">
              ...
              <button @click="loadData" class="btn-refresh-pro" title="Actualiser" :disabled="loading">
                <i class="fa-solid fa-rotate" :class="{ 'fa-spin': loading }"></i>
              </button>
              <div class="counter-badge-pro">
                <i class="fa-solid fa-users me-2"></i>
                <span class="fw-900">{{ filteredStaff.length }}</span>
                <span class="badge-label">MEMBRES</span>
              </div>
            </div>
          </header>
          ...
          <div v-if="loading" class="text-center py-5">
            ...
            <p class="mt-3 text-muted small fw-700">Synchronisation des données...</p>
          </div>
            ...
            <div class="enigma-card overflow-hidden">
              <div class="table-header-elite">
                <div class="d-flex align-items-center gap-2">
                  <div class="table-header-icon"><i class="fa-solid fa-users"></i></div>
                  <span>RÉPERTOIRE DES COLLABORATEURS</span>
                </div>
                <div class="table-badge-count">{{ paginatedStaff.length }} / {{ filteredStaff.length }}</div>
              </div>
                  ...
                  <tbody>
                    <tr v-for="member in paginatedStaff" :key="member.id">
                      <td>
                        <div class="d-flex align-items-center gap-3">
                          <div class="staff-avatar">
                            <img v-if="member.photoUrl" :src="getPhotoUrl(member.photoUrl)" class="avatar-img-fit" alt="Photo" />
                            <span v-else>{{ member.prenom?.charAt(0) }}{{ member.nomFamille?.charAt(0) }}</span>
                          </div>
                          <div>
                            <div class="fw-800 text-dark">{{ member.prenom }} {{ member.nomFamille }}</div>
                      ...
                      <td class="text-end">
                        ...
                        <span v-else class="consultation-badge">Lecture seule</span>
                      </td>
                    </tr>
                    <tr v-if="filteredStaff.length === 0">
                      <td colspan="5" class="empty-state-row">
                        <div class="empty-icon"><i class="fa-solid fa-user-slash"></i></div>
                        <p>Aucun membre trouvé pour votre entreprise.</p>
                      </td>
                    </tr>
                  </tbody>
                </table>
              ...
              <div v-if="totalPages > 1" class="pagination-bar">
                <div class="pagination-info">
                  <span>Affichage</span>
                  <strong>{{ currentPage * perPage + 1 }}–{{ Math.min((currentPage + 1) * perPage, filteredStaff.length) }}</strong>
                  <span>sur</span>
                  <strong>{{ filteredStaff.length }}</strong>
                  <span>membres</span>
                </div>
                <div class="pagination-controls">
                  <button class="pg-btn" @click="currentPage = 0" :disabled="currentPage === 0" title="Première page">
                    <i class="fa-solid fa-angles-left"></i>
                  </button>
                  <button class="pg-btn" @click="currentPage--" :disabled="currentPage === 0" title="Précédente">
                  ...
                  <button class="pg-btn" @click="currentPage++" :disabled="currentPage >= totalPages - 1" title="Suivante">
                    <i class="fa-solid fa-angle-right"></i>
                  </button>
                  <button class="pg-btn" @click="currentPage = totalPages - 1" :disabled="currentPage >= totalPages - 1" title="Dernière page">
          ...
          <div v-else-if="viewMode === 'grid'" class="animate__animated animate__fadeIn">
            <div class="row g-4">
              ...
              <div v-else v-for="member in paginatedStaff" :key="member.id" class="col-xl-3 col-lg-4 col-md-6">
                <div class="staff-grid-card">
                  <div class="grid-card-top">
                    ...
                    <div class="d-flex gap-1" v-if="isAdmin">
                      <button @click="openEditModal(member)" class="btn-icon-sm sm" title="Modifier">
                        <i class="fa-solid fa-pen-to-square"></i>
                      </button>
                      <button @click="deleteMemberConfirm(member)" class="btn-icon-sm danger sm" title="Supprimer">
            ...
            <div v-if="totalPages > 1" class="enigma-card mt-4 overflow-hidden">
              <div class="pagination-bar border-0">
                ...
                <div class="pagination-controls">
                  <button class="pg-btn" @click="currentPage = 0" :disabled="currentPage === 0"><i class="fa-solid fa-angles-left"></i></button>
                  <button class="pg-btn" @click="currentPage--" :disabled="currentPage === 0"><i class="fa-solid fa-angle-left"></i></button>
                  <div class="pg-numbers">
                    <button
                      v-for="p in pagesRange"
                      :key="p"
                  ...
                  <button class="pg-btn" @click="currentPage++" :disabled="currentPage >= totalPages - 1"><i class="fa-solid fa-angle-right"></i></button>
                  <button class="pg-btn" @click="currentPage = totalPages - 1" :disabled="currentPage >= totalPages - 1"><i class="fa-solid fa-angles-right"></i></button>
                </div>
                <div class="pagination-size">
                  <span class="pg-size-label">Lignes</span>
                  <select v-model="perPage" @change="currentPage = 0" class="pg-size-select">
...
const filteredStaff = computed(() => {
  let list = staff.value.filter(member => {
    ...
    const matchSearch  = `${member.prenom} ${member.nomFamille}`.toLowerCase().includes(search) || member.email.toLowerCase().includes(search);
    const matchRole    = selectedRoleFilter.value === '' || member.roleNom === selectedRoleFilter.value;
    return matchSearch && matchRole;
  });

  list.sort((a, b) => {
    if (sortBy.value === 'name') return `${a.prenom} ${a.nomFamille}`.localeCompare(`${b.prenom} ${b.nomFamille}`);
    if (sortBy.value === 'date') return new Date(b.creeLe || 0) - new Date(a.creeLe || 0);
    if (sortBy.value === 'role') return (a.roleNom || '').localeCompare(b.roleNom || '');
    return 0;
  });
...
const totalPages    = computed(() => Math.ceil(filteredStaff.value.length / perPage.value));
...
const kpiStats = computed(() => {
  const roles     = [...new Set(staff.value.filter(m => m.roleNom !== 'SuperAdmin').map(m => m.roleNom))];
  const admins    = staff.value.filter(m => m.roleNom?.toLowerCase().includes('admin')).length;
  const evaluateurs = staff.value.filter(m => m.roleNom === 'Evaluateur').length;
  return [
    { label: 'Total Personnel',   value: filteredStaff.value.length, icon: 'fa-solid fa-users',        color: '#6366f1', bg: '#eef2ff' },
    { label: 'Rôles Actifs',      value: filteredRolesList.value.length, icon: 'fa-solid fa-id-badge', color: '#f59e0b', bg: '#fffbeb' },
    { label: 'Administrateurs',   value: admins,                     icon: 'fa-solid fa-shield-halved', color: '#10b981', bg: '#ecfdf5' },
    { label: 'Évaluateurs',       value: evaluateurs,                icon: 'fa-solid fa-user-check',   color: '#f43f5e', bg: '#fff1f2' },
  ];
});

/* ─── RESET PAGE ON FILTER ───────────────────────────────────────── */
watch([searchQuery, selectedRoleFilter, sortBy], () => { currentPage.value = 0; });
...
const loadData = async () => {
  ...
  try {
    ...
    showToast('Erreur lors du chargement des données.', 't-error', 'fa-solid fa-circle-exclamation');
  } finally {
    loading.value = false;
  }
};
...
const handleSave = async () => {
  ...
  try {
    ...
    showToast('Membre mis à jour avec succès.', 't-success', 'fa-solid fa-circle-check');
    loadData();
  } catch (err) {
    editError.value = err.response?.data?.message || 'Erreur lors de la mise à jour.';
  } finally {
    saving.value = false;
  }
};
...
const deleteMemberConfirm = (member) => {
  showConfirmDialog(
    ...
    `${member.prenom} ${member.nomFamille} sera définitivement retiré du personnel.`,
    'fa-solid fa-trash-can',
    async () => {
      try {
        await api.delete(`/Staff/${member.id}`);
        showToast('Collaborateur supprimé.', 't-warn', 'fa-solid fa-trash-can');
        loadData();
      } catch {
        showToast('Erreur lors de la suppression.', 't-error', 'fa-solid fa-circle-exclamation');
      }
    }
  );
};
...
/* ═══════════════════ HEADER ═══════════════════ */
.premium-title { font-weight: 900; font-size: 2.2rem; letter-spacing: -1px; margin: 0; line-height: 1.1; }
.gradient-text {
...
.btn-refresh-pro:hover:not(:disabled) { background: #f8fafc; border-color: #f59e0b; color: #f59e0b; }
...
.stat-label { font-size: 0.68rem; font-weight: 700; color: #94a3b8; text-transform: uppercase; margin-top: 4px; letter-spacing: 0.5px; }
...
.search-inline-input {
  ...
  font-weight: 700; font-size: 0.88rem; width: 100%; font-family: inherit; color: #0f172a;
}
...
.filter-icon { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); font-size: 12px; color: #94a3b8; pointer-events: none; }
.filter-select-pro {
...
.elite-table thead th {
  ...
  letter-spacing: 1.5px; padding: 14px 24px; text-transform: uppercase; background: transparent;
}
.elite-table td { padding: 18px 24px; border-top: 1px solid #f8fafc; vertical-align: middle; }
.elite-table tbody tr { transition: background 0.2s; }
.elite-table tbody tr:hover { background: #fafafa; }

/* AVATAR */
.staff-avatar {
  width: 44px; height: 44px; background: linear-gradient(135deg, #f59e0b, #fbbf24);
  color: #0f172a; border-radius: 14px; display: flex; align-items: center; justify-content: center;
  font-weight: 900; font-size: 14px; flex-shrink: 0; overflow: hidden;
  box-shadow: 0 4px 10px rgba(245,158,11,0.2);
}
.staff-avatar.large { width: 72px; height: 72px; font-size: 22px; border-radius: 22px; }
.avatar-img-fit { width: 100%; height: 100%; object-fit: cover; }

.member-status-dot { display: flex; align-items: center; gap: 5px; font-size: 10px; font-weight: 800; color: #10b981; text-transform: uppercase; letter-spacing: 0.5px; margin-top: 3px; }
.status-dot-green { width: 6px; height: 6px; background: #10b981; border-radius: 50%; }

/* ROLE BADGES */
.role-badge {
  padding: 5px 12px; border-radius: 10px; font-size: 11px; font-weight: 800; letter-spacing: 0.3px;
...
.date-cell { font-size: 11px; font-weight: 800; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px; }
.consultation-badge { font-size: 10px; font-weight: 700; color: #94a3b8; font-style: italic; }
...
.grid-avatar-wrap { position: relative; display: flex; justify-content: center; padding: 8px 0; }
.grid-status-ring {
...
.enigma-field:focus { border-color: #f59e0b; background: white; box-shadow: 0 0 0 4px rgba(251,191,36,0.1); }
...
/* BUTTONS */
.btn-enigma-primary {
  background: #0f172a; color: white; border: none; padding: 13px 24px;
  border-radius: 16px; font-weight: 800; position: relative; overflow: hidden;
  cursor: pointer; font-family: inherit; transition: 0.3s;
}
.btn-enigma-primary .btn-glow {
  position: absolute; inset: 0; background: linear-gradient(135deg, #f59e0b, #fbbf24);
...
.btn-qv-cancel {
  ...
  padding: 12px 24px; border-radius: 14px; font-weight: 800; cursor: pointer; font-family: inherit;
}
...
.pg-num:hover:not(:disabled):not(.ellipsis) { background: #f1f5f9; border-color: #e2e8f0; color: #0f172a; }
.pg-num.active {
...
@keyframes zoomInM { from { opacity: 0; transform: scale(0.92); } to { opacity: 1; transform: scale(1); } }
...
@media (max-width: 576px) {
  ...
  .pagination-info, .pagination-size { display: none; }
}
