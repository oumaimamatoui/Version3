  ...
  <div class="enigma-master-root d-flex overflow-hidden" :data-theme="currentTheme">
      ...
      <main class="canvas-engine flex-grow-1 overflow-auto custom-scrollbar">
        <div class="dashboard-view animate__animated animate__fadeIn p-4 p-lg-5">
          ...
          <header class="d-flex justify-content-between align-items-end mb-5 flex-wrap gap-3">
            <div>
              <div class="breadcrumb-pro mb-2">
                ...
                <span class="current">Utilisateurs Système</span>
              </div>
              <h2 class="premium-title">Gestion des <span class="gradient-text">Accès</span></h2>
              <p class="text-muted-pro small m-0">Gérez l'ensemble des accès et les privilèges des organisations.</p>
            </div>
            <div class="d-flex gap-3 flex-wrap align-items-center">
              <!-- Theme Toggle -->
              <button class="btn-theme-toggle" @click="toggleTheme" :title="currentTheme === 'dark' ? 'Mode clair' : 'Mode sombre'">
              ...
              <div class="view-toggle-cluster">
                ...
                <button :class="['btn-view-toggle', { active: viewMode === 'analytics' }]" @click="viewMode = 'analytics'" title="Analytique">
                  <i class="fa-solid fa-chart-simple"></i>
                </button>
              </div>
              <button @click="fetchData" class="btn-refresh-pro" :disabled="loading" title="Actualiser">
          ...
          <div class="row g-4 mb-5">
            <div class="col-xl-4 col-md-6" v-for="stat in platformStats" :key="stat.label">
            ...
            <div class="row g-4">
              <div class="col-lg-8">
                <div class="analytics-card-pro p-4">
                  <div class="d-flex justify-content-between align-items-center mb-4">
                    <h6 class="fw-800 m-0">Distribution des Rôles</h6>
                    <div class="d-flex gap-2 align-items-center">
                      <span class="legend-dot dot-amber"></span><span class="small text-muted">Actifs</span>
                      <span class="legend-dot dot-indigo ms-3"></span><span class="small text-muted">Inactifs</span>
                    </div>
                  </div>
              ...
              <div class="col-lg-4">
                <div class="analytics-card-pro p-4 h-100">
                  <h6 class="fw-800 mb-4">Statut Global</h6>
          ...
          <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            ...
            <div class="d-flex gap-2">
              <div class="search-inline-box">
                ...
                <input type="text" v-model="searchQuery" placeholder="Rechercher..." class="search-inline-input">
          ...
          <div v-if="viewMode === 'grid'" class="row g-4">
            ...
            <div v-else-if="filteredUsers.length === 0" class="col-12">
              <div class="empty-state-pro py-5 text-center">
                ...
                <h5 class="fw-800">Aucun utilisateur trouvé</h5>
                <p class="text-muted">Modifiez les filtres ou la recherche.</p>
              </div>
            </div>
            <div v-else v-for="u in filteredUsers" :key="u.id" class="col-xl-4 col-md-6 animate__animated animate__fadeInUp">
              <div class="user-card-modern">
                <div class="card-header-modern mb-3 d-flex justify-content-between align-items-start">
                  <span class="status-badge-user" :class="u.isActive ? 'status-active' : 'status-inactive'">
                    <span class="status-dot"></span> {{ u.isActive ? 'Actif' : 'Inactif' }}
                  </span>
                  <div class="dropdown">
                    ...
                    <ul class="dropdown-menu border-0 shadow-premium p-2 rounded-4">
                      <li><button class="dropdown-item rounded-3" @click="handleToggleStatus(u)">
                        <i class="fa-solid fa-toggle-on me-2"></i>Changer statut
                      </button></li>
                      <li><hr class="dropdown-divider"></li>
                      <li><button class="dropdown-item rounded-3 text-danger" @click="handleDelete(u)">
                ...
                <div class="d-flex align-items-center gap-2 mb-3">
                  <span class="org-badge"><i class="fa-solid fa-building me-1 opacity-50"></i>{{ u.org }}</span>
                </div>
                ...
                <div class="card-footer-modern d-flex justify-content-between align-items-center pt-3 border-top border-light">
                  ...
                  <div class="d-flex align-items-center gap-2">
                    ...
                    <button @click="handleDelete(u)" class="btn-icon-sm danger"><i class="fa-solid fa-trash-can"></i></button>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="viewMode === 'table'" class="animate__animated animate__fadeIn">
            <div v-if="loading" class="text-center py-5"><div class="spinner-pro-premium"></div></div>
            <template v-else>
              <!-- List Header -->
              <div class="list-header-row d-flex align-items-center px-4 py-2 mb-2">
                <span style="flex:2" class="list-col-label">UTILISATEUR</span>
                <span style="flex:1.5" class="list-col-label">ORGANISATION</span>
                <span style="width:130px" class="list-col-label">RÔLE</span>
                <span style="width:140px" class="list-col-label">DERNIÈRE CONNEXION</span>
                <span style="width:80px"  class="list-col-label text-center">ÉTAT</span>
                <span style="width:60px"  class="list-col-label text-center">ACTION</span>
              </div>
              <div v-if="filteredUsers.length === 0" class="text-center py-4 text-muted">Aucun utilisateur trouvé.</div>
              <div v-for="u in filteredUsers" :key="u.id" class="list-row-item d-flex align-items-center px-4 py-3 mb-2">
                ...
                <div style="width:60px" class="d-flex gap-1 justify-content-center">
                  <button @click="handleDelete(u)" class="btn-icon-sm danger" title="Supprimer"><i class="fa-solid fa-trash-can"></i></button>
                </div>
              </div>
              <!-- Footer -->
              <div class="table-footer-bar mt-3 d-flex justify-content-between align-items-center px-2">
                <span class="tiny text-muted fw-bold uppercase">Affichage de {{ filteredUsers.length }} / {{ users.length }} utilisateurs</span>
    ...
    <transition name="toast-slide">
      <div v-if="globalToast.active" class="enigma-toast" :class="globalToast.type">
        ...
        <div class="t-body"><strong>SYSTÈME</strong><p class="m-0 small">{{ globalToast.message }}</p></div>
      </div>
    </transition>
  </div>
</template>
...
const platformStats = ref([
  ...
  { label: 'Sessions en cours',      val: '0', icon: 'fa-solid fa-bolt-lightning',           bg: 'rgba(245,158,11,0.1)',  color: '#f59e0b', trend: 'Live', key: 'sessionsIARecentes' },
]);
...
const filterTabs = computed(() => [
  ...
  { label: 'Actifs',   value: 'active', count: users.value.filter(u => u.isActive).length },
  { label: 'Inactifs', value: 'inactive', count: users.value.filter(u => !u.isActive).length },
]);
...
const filteredUsers = computed(() => {
  return users.value.filter(u => {
    ...
    const matchRole = selectedRole.value === 'Tous les rôles' || u.role === selectedRole.value;
...
const donutSegments = computed(() => {
  ...
  const circ     = 283;
  return [
    { label: 'Actifs',   count: active,   color: '#10b981', dash: (active/total)*circ,   offset: circ/4 },
    { label: 'Inactifs', count: inactive, color: '#f43f5e', dash: (inactive/total)*circ, offset: circ/4 - (active/total)*circ },
  ];
});
  ...
  showConfirmDialog(
    ...
    async () => {
      try {
        ...
        showPulseToast("Utilisateur supprimé.", 'success', 'fa-solid fa-check');
        fetchData();
      } catch { showPulseToast("Erreur lors de la suppression.", 'error', 'fa-solid fa-xmark'); }
    }
  );
};
...
const handleToggleStatus = async (user) => {
  try {
    ...
    showPulseToast(`Statut de ${user.name} mis à jour.`, 'success', 'fa-solid fa-check');
  } catch {
    showPulseToast("Erreur changement de statut.", 'error', 'fa-solid fa-xmark');
    fetchData();
  }
};
...
const showConfirmDialog = (title, message, icon, cb) => Object.assign(confirmDialog, { title, message, icon, _cb: cb, show: true });
...
.btn-refresh-pro:hover:not(:disabled) { background: #f8fafc; border-color: #f59e0b; color: #f59e0b; transform: rotate(180deg); }
...
.stat-trend { display: flex; align-items: center; gap: 4px; font-size: 0.65rem; font-weight: 800; color: #f59e0b; background: #fffbeb; padding: 4px 10px; border-radius: 8px; }
...
.search-inline-input {
  ...
  padding: 10px 0; font-weight: 700; font-size: 0.85rem; width: 180px; font-family: inherit;
}
...
.sort-select-pro {
  ...
  font-weight: 700; font-size: 0.8rem; background: white; outline: none; cursor: pointer; font-family: inherit;
}
...
.user-card-modern:hover { transform: translateY(-8px); border-color: #f59e0b; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.08); }
...
.status-badge-user {
  ...
  font-weight: 800; text-transform: uppercase; display: inline-flex; align-items: center;
}
...
/* ══════════════════════════════════════════════
   TOAST
══════════════════════════════════════════════ */
.enigma-toast { position: fixed; bottom: 30px; right: 30px; background: #0f172a; color: white; padding: 18px 28px; border-radius: 18px; display: flex; align-items: center; gap: 14px; z-index: 3000; border-left: 5px solid #f59e0b; box-shadow: 0 20px 40px rgba(0,0,0,0.2); }
